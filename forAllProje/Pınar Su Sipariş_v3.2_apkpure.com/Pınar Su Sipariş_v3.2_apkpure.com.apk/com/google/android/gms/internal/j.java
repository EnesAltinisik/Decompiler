...
class j {
    ...
    public void a(String str) {
        ...
        if (intValue == 407) {
            throw new zzaig("connection failed: proxy authentication not supported");
        } else if (intValue == WalletConstants.ERROR_CODE_INVALID_PARAMETERS) {
            throw new zzaig("connection failed: 404 not found");
        } else if (intValue != 101) {
            throw new zzaig("connection failed: unknown status code " + intValue);
        }
    }

    public void a(HashMap<String, String> hashMap) {
        if (!((String) hashMap.get("Upgrade")).toLowerCase(Locale.US).equals("websocket")) {
            throw new zzaig("connection failed: missing header field in server handshake: Upgrade");
        } else if (!((String) hashMap.get("Connection")).toLowerCase(Locale.US).equals("upgrade")) {
            throw new zzaig("connection failed: missing header field in server handshake: Connection");
        }
    }
