...
public class zzajp {
    public static boolean equals(Object obj, Object obj2) {
        return zzz.equal(obj, obj2);
    }

    public static Uri zzc(FirebaseApp firebaseApp, String str) throws UnsupportedEncodingException {
        if (TextUtils.isEmpty(str)) {
            return null;
        }
        if (str.toLowerCase().startsWith("gs://")) {
            String str2 = "gs://";
            String valueOf = String.valueOf(zzajl.zziH(zzajl.zziJ(str.substring(5))));
            return Uri.parse(valueOf.length() != 0 ? str2.concat(valueOf) : new String(str2));
        }
        Uri parse = Uri.parse(str);
        valueOf = parse.getScheme().toLowerCase();
        if (equals(valueOf, "http") || equals(valueOf, Constants.SCHEME)) {
            try {
                int indexOf = parse.getAuthority().toLowerCase().indexOf(zzg(firebaseApp));
                valueOf = zzajl.zziI(parse.getEncodedPath());
                ...
                zzaa.zzh(str2, "No bucket specified");
    ...
    private static String zzg(FirebaseApp firebaseApp) throws RemoteException {
        return zzajs.zzi(firebaseApp).zzVv();
    }
    ...
    public static long zziK(String str) {
        if (str == null) {
            return 0;
        }
