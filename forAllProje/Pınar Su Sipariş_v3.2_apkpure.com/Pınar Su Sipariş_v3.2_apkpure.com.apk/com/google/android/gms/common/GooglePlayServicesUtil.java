...
public final class GooglePlayServicesUtil extends zze {
    ...
    private static void zza(int i, Context context, String str, PendingIntent pendingIntent) {
        ...
        if (com.google.android.gms.common.util.zzi.zzaB(context)) {
            zzaa.zzai(zzs.zzvb());
            build = new Notification.Builder(context).setSmallIcon(R.drawable.common_ic_googleplayservices).setPriority(2).setAutoCancel(true).setStyle(new BigTextStyle().bigText(new StringBuilder((String.valueOf(zzg).length() + 1) + String.valueOf(zzc).length()).append(zzg).append(" ").append(zzc).toString())).addAction(R.drawable.common_full_open_on_phone, resources.getString(R.string.common_open_on_phone), pendingIntent).build();
        } else {
