...
class NotificationCompatImplBase {
    ...
    private static <T extends Action> RemoteViews generateContentView(Context context, CharSequence charSequence, CharSequence charSequence2, CharSequence charSequence3, int i, Bitmap bitmap, CharSequence charSequence4, boolean z, long j, List<T> list, int[] iArr, boolean z2, PendingIntent pendingIntent) {
        int i2;
        RemoteViews applyStandardTemplate = applyStandardTemplate(context, charSequence, charSequence2, charSequence3, i, bitmap, charSequence4, z, j, h.notification_template_media, true);
        int size = list.size();
        if (iArr == null) {
            i2 = 0;
        } else {
        ...
        applyStandardTemplate.removeAllViews(f.media_actions);
        if (i2 > 0) {
            for (int i3 = 0; i3 < i2; i3++) {
                ...
                applyStandardTemplate.addView(f.media_actions, generateMediaActionButton(context, (Action) list.get(iArr[i3])));
            }
        }
        if (z2) {
            applyStandardTemplate.setViewVisibility(f.end_padder, 8);
            applyStandardTemplate.setViewVisibility(f.cancel_action, 0);
            applyStandardTemplate.setOnClickPendingIntent(f.cancel_action, pendingIntent);
            applyStandardTemplate.setInt(f.cancel_action, "setAlpha", context.getResources().getInteger(g.cancel_button_image_alpha));
        } else {
            applyStandardTemplate.setViewVisibility(f.end_padder, 0);
            applyStandardTemplate.setViewVisibility(f.cancel_action, 8);
        }
    ...
    private static <T extends Action> RemoteViews generateBigContentView(Context context, CharSequence charSequence, CharSequence charSequence2, CharSequence charSequence3, int i, Bitmap bitmap, CharSequence charSequence4, boolean z, long j, List<T> list, boolean z2, PendingIntent pendingIntent) {
        ...
        if (min > 0) {
            for (int i2 = 0; i2 < min; i2++) {
                applyStandardTemplate.addView(f.media_actions, generateMediaActionButton(context, (Action) list.get(i2)));
            }
        }
        if (z2) {
            applyStandardTemplate.setViewVisibility(f.cancel_action, 0);
    ...
    private static RemoteViews generateMediaActionButton(Context context, Action action) {
        ...
        remoteViews.setImageViewResource(f.action0, action.getIcon());
        if (obj == null) {
            remoteViews.setOnClickPendingIntent(f.action0, action.getActionIntent());
        }
        if (VERSION.SDK_INT >= 15) {
            remoteViews.setContentDescription(f.action0, action.getTitle());
        }
        return remoteViews;
    }
