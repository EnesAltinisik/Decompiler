...
public final class DiskLruCache implements Closeable, Flushable {
    ...
    private synchronized void completeEdit(Editor editor, boolean z) throws IOException {
        int i = 0;
        synchronized (this) {
            Entry access$1700 = editor.entry;
            if (access$1700.currentEditor != editor) {
                throw new IllegalStateException();
            }
            if (z) {
                if (!access$1700.readable) {
                    int i2 = 0;
                    while (i2 < this.valueCount) {
                        if (!editor.written[i2]) {
                            editor.abort();
                            throw new IllegalStateException("Newly created entry didn't create value for index " + i2);
                        } else if (!this.fileSystem.exists(access$1700.dirtyFiles[i2])) {
                            editor.abort();
                            break;
                        } else {
                            i2++;
            ...
            while (i < this.valueCount) {
                File file = access$1700.dirtyFiles[i];
                if (!z) {
                    this.fileSystem.delete(file);
                } else if (this.fileSystem.exists(file)) {
                    File file2 = access$1700.cleanFiles[i];
                    this.fileSystem.rename(file, file2);
                    long j = access$1700.lengths[i];
                    long size = this.fileSystem.size(file2);
                    access$1700.lengths[i] = size;
