...
public class SquareShapeRenderer implements IShapeRenderer {
    public void renderShape(Canvas canvas, IScatterDataSet iScatterDataSet, ViewPortHandler viewPortHandler, float f, float f2, Paint paint) {
        ...
        if (((double) scatterShapeSize) > 0.0d) {
            ...
            canvas.drawRect((f - convertDpToPixel) - f5, (f2 - convertDpToPixel) - f5, (f + convertDpToPixel) + f5, (f2 + convertDpToPixel) + f5, paint);
            if (scatterShapeHoleColor != ColorTemplate.COLOR_NONE) {
                paint.setStyle(Style.FILL);
                paint.setColor(scatterShapeHoleColor);
                canvas.drawRect(f - convertDpToPixel, f2 - convertDpToPixel, f + convertDpToPixel, f2 + convertDpToPixel, paint);
                return;
            }
            return;
        }
