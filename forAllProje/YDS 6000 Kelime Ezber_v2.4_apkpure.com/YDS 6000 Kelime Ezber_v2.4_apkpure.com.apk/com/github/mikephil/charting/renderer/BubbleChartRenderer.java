            ...
            for (int i = 0; i < dataSets.size(); i++) {
                ...
                if (shouldDrawValues(iBubbleDataSet)) {
                    ...
                    for (int i2 = 0; i2 < generateTransformedValuesBubble.length; i2 += 2) {
                        ...
                        phaseY = generateTransformedValuesBubble[i2 + 1];
                        if (!this.mViewPortHandler.isInBoundsRight(f2)) {
                            break;
                        }
                        if (this.mViewPortHandler.isInBoundsLeft(f2) && this.mViewPortHandler.isInBoundsY(phaseY)) {
                            BubbleEntry bubbleEntry = (BubbleEntry) iBubbleDataSet.getEntryForIndex((i2 / 2) + this.mXBounds.min);
                            drawValue(canvas, iBubbleDataSet.getValueFormatter(), bubbleEntry.getSize(), bubbleEntry, i, f2, phaseY + (0.5f * calcTextHeight), argb);
                        }
                    }
                }
            }
