...
public abstract class PieRadarChartBase<T extends ChartData<? extends IDataSet<? extends Entry>>> extends Chart<T> {
    ...
    public float distanceToCenter(float f, float f2) {
        ...
        float sqrt = (float) Math.sqrt(Math.pow((double) (f2 > centerOffsets.y ? f2 - centerOffsets.y : centerOffsets.y - f2), 2.0d) + Math.pow((double) (f > centerOffsets.x ? f - centerOffsets.x : centerOffsets.x - f), 2.0d));
