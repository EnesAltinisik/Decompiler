package com.google.android.gms.b;

import android.os.Parcel;
import android.os.Parcelable.Creator;
import com.google.android.gms.common.internal.safeparcel.a;

@vz
public final class aab extends a {
    public static final Creator<aab> CREATOR = new aac();
    public final int a;
    public String b;
    public int c;
    public int d;
    public boolean e;

    public aab(int i, int i2, boolean z) {
        this(i, i2, z, false);
    }

    public aab(int i, int i2, boolean z, boolean z2) {
        String valueOf = String.valueOf("afma-sdk-a-v");
        String str = z ? "0" : z2 ? "2" : "1";
        this(1, new StringBuilder((String.valueOf(valueOf).length() + 24) + String.valueOf(str).length()).append(valueOf).append(i).append(".").append(i2).append(".").append(str).toString(), i, i2, z);
    }

    aab(int i, String str, int i2, int i3, boolean z) {
        this.a = i;
        this.b = str;
        this.c = i2;
        this.d = i3;
        this.e = z;
    }

    public static aab a() {
        return new aab(10084208, 10084208, true);
    }

    public void writeToParcel(Parcel parcel, int i) {
        aac.a(this, parcel, i);
    }
}
