...
class ke {
    ...
    public ke(URI uri, String str, Map<String, String> map) {
        this.a = uri;
        this.b = str;
        this.d = map;
        this.c = b();
    ...
    public void a(String str) {
        ...
        if (intValue == 407) {
            throw new kd("connection failed: proxy authentication not supported");
        } else if (intValue == 404) {
            throw new kd("connection failed: 404 not found");
        } else if (intValue != 101) {
            throw new kd("connection failed: unknown status code " + intValue);
        }
    }

    public void a(HashMap<String, String> hashMap) {
        if (!((String) hashMap.get("Upgrade")).toLowerCase(Locale.US).equals("websocket")) {
            throw new kd("connection failed: missing header field in server handshake: Upgrade");
        } else if (!((String) hashMap.get("Connection")).toLowerCase(Locale.US).equals("upgrade")) {
            throw new kd("connection failed: missing header field in server handshake: Connection");
        }
    }
